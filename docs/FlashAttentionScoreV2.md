...
- **参数说明：**
  ...
  - attenMaskOptional（aclTensor\*，计算输入）：Device侧的aclTensor，公式中的atten_mask，可选参数，数据类型支持BOOL、UINT8，[数据格式](common/数据格式.md)支持ND，输入shape类型需为\[B,N,S,S\]、\[B,1,S,S\]、\[1,1,S,S\]、\[S,S\]，varlen场景只支持SS格式，SS分别是maxSq和maxSkv；综合约束请见[约束与限制](#1)。
  ...
  - headNum（int64\_t，计算输入）：Host侧的int64_t，数据类型支持，INT64，代表单卡的head个数；综合约束请见[约束与限制](#1)。
