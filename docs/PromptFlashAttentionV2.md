...
- **参数说明：**
  ...
  - pseShift（aclTensor\*，计算输入）：预留参数，暂未使用。Device侧的aclTensor，数据类型支持FLOAT16、BFLOAT16，数据类型与query的数据类型需满足数据类型推导规则。不支持[非连续的Tensor](common/非连续的Tensor.md)，[数据格式](common/数据格式.md)支持ND。目前该参数会被强制设置为nullptr，**Atlas推理系列加速卡产品仅支持nullptr**。
