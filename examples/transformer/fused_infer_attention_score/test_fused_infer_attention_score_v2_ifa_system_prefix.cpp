...
/*!
 * \file test_fused_infer_attention_score_v2_ifa_prefix.cpp
 * \brief
 */
...
int main()
{
    ...
    void *keySharedPrefixDeviceAddr = nullptr;
    void *valueSharedPrefixDeviceAddr = nullptr;
    ...
    aclTensor *keySharedPrefixTensor = nullptr;
    aclTensor *valueSharedPrefixTensor = nullptr;
    ...
    std::vector<uint16_t> keySharedPrefixHostData(keySharedPrefixShapeSize, 0x3C00);
    std::vector<uint16_t> valueSharedPrefixHostData(valueSharedPrefixShapeSize, 0x3C00);
    ...
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // Create keySharedPrefix aclTensor.
    ret = CreateAclTensor(keySharedPrefixHostData, keySharedPrefixShape, &keySharedPrefixDeviceAddr,
                          aclDataType::ACL_FLOAT16, &keySharedPrefixTensor);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // Create valueSharedPrefix aclTensor.
    ret = CreateAclTensor(valueSharedPrefixHostData, valueSharedPrefixShape, &valueSharedPrefixDeviceAddr,
                          aclDataType::ACL_FLOAT16, &valueSharedPrefixTensor);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    aclrtFree(keySharedPrefixDeviceAddr);
    aclrtFree(valueSharedPrefixDeviceAddr);
    if (workspaceSize > 0) {
