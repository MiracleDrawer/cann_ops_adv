...
void FlashAttentionScoreGradTilingS1s2Bn2gs1s2SameAb::SetQKVStartIdx() {
    ...
    if (qStartIdxShape.GetDimNum() != 1) {
        OPS_LOG_W(context_, "[%s]qStartIdxShape is invalid %ld %ld", TEMPLATE_NAME_SAME_AB, qStartIdxShape.GetDimNum(),
    ...
    if (kvStartIdxShape.GetDimNum() != 1) {
        OPS_LOG_W(context_, "[%s]kvStartIdxShape is invalid %ld %ld", TEMPLATE_NAME_SAME_AB, kvStartIdxShape.GetDimNum(),
