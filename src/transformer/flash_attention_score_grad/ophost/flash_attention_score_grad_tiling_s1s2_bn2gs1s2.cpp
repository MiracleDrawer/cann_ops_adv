...
uint64_t FlashAttentionScoreGradTilingS1s2Bn2gs1s2::GetTilingKey() const
{
    ...
    auto mm1IsNZOut =
        fBaseParams.mm1IsNZOut && unpackValue != OptionEnum::ENABLE ? OptionEnum::ENABLE : OptionEnum::DISABLE;
...
void FlashAttentionScoreGradTilingS1s2Bn2gs1s2::SetQKVStartIdx() {
    ...
    if (qStartIdxShape.GetDimNum() != 1) {
        OPS_LOG_W(context_, "[%s]qStartIdxShape is invalid %ld %ld", templateNameS1S2, qStartIdxShape.GetDimNum(),
    ...
    if (kvStartIdxShape.GetDimNum() != 1) {
        OPS_LOG_W(context_, "[%s]kvStartIdxShape is invalid %ld %ld", templateNameS1S2, kvStartIdxShape.GetDimNum(),
...
{
    OPS_LOG_I(context_,
              "FAG s1s2_bn2gs1s2 with shape b[%u] n2[%u] g[%u] s1[%u] s2[%u] d[%u] preToken[%d] nextToken[%d]!",
...
std::tuple<uint32_t, uint32_t, uint32_t> FlashAttentionScoreGradTilingS1s2Bn2gs1s2::FuzzyForBestSplit()
{
    ...
    OPS_LOG_D(context_, "s1Inner = %d, s2Inner = %d, tmpBufferSize = %d", first, second, tmpBufferSize);
...
{
    OPS_LOG_D(context_, "Enter s1Inner = %d, s1Inner = %d", s1Inner, s2Inner);
...
void FlashAttentionScoreGradTilingS1s2Bn2gs1s2::GetParseS1S2OuterInfo(int64_t (*parseInfo)[ARRAY_LENGTH])
{
    for (int64_t i = 0; i < fBaseParams.s2Outer; i++) {
        ...
        OPS_LOG_D(context_, "idx = %d: Begin = %d, End = %d, Length = %d, total_Length = %d", i, parseInfo[i][0],
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2gs1s2::GetSparseBlockInfo()
{
    ...
    OPS_LOG_D(context_, "Sparse parseInfo fusedOuter = %d: blockFactor = %d, blockTail = %d", fusedOuter, blockFactor,
              blockTail);
    fBaseParams.blockOuter = blockOuter;
    fBaseParams.blockFactor = blockFactor;
    ...
    for (int64_t c = 1; c < blockOuter; c++) {
        ...
        for (int64_t i = 0; i < fBaseParams.s2Outer; i++) {
            if (gTail >= preSize) {
                ...
                if (gTail < nextSize) {
                    ...
                    OPS_LOG_D("Sparse", " s1oIdx = %d, s2oIdx = %d, preSize = %d, nextSize = %d", s1oIdx, s2oIdx,
                              preSize, nextSize);
                    break;
                }
        ...
        OPS_LOG_D(context_, "blockStarts[c] = %d:", blockStarts[c]);
    }
    ...
    const int64_t bIdx, const int64_t prefixN, std::vector<std::vector<std::pair<int64_t, int64_t>>> &s1ValidIdx)
{
    for (int64_t i = 0; i < bIdx; ++i) {
        if (fBaseParams.prefixN[i] == prefixN) {
            OPS_LOG_D(context_, "prefixN of bIdx[%u] and bIdx[%u] is same as %u", i, bIdx, prefixN);
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2gs1s2::GetSparsePrefixBlockInfo()
{
    ...
    OPS_LOG_D(context_, "Sparse parseInfo totalValidBaseBlock = %lu: blockFactor = %u, blockOuter = %u",
    ...
    std::vector<std::vector<int64_t>> &totalBlockInfo)
{
    ...
    OPS_LOG_D(context_, "SparseMode %u, find band index %u", fBaseParams.sparseMode, fBaseParams.bandIdx);
    ...
    for (int64_t i = 0; i < fBaseParams.b; i++) {
        ...
        for (int64_t j = 0; j < fBaseParams.s2Outer; j++) {
            ...
            OPS_LOG_D(context_, "s2Outer idx = %d: Begin = %d, End = %d, Sum_S1S2 = %d", j,
        ...
        OPS_LOG_D(context_, "Up to b idx = %d , a total of %d blocks that need to be calculated", i,
                  calculatedBlockInfo[i][0][SUM_ALL]);
    }
}
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2gs1s2::GetSparseUnpadBlockInfo()
{
    ...
    OPS_LOG_D(context_, "fusedOuter = %u: blockFactor = %u, blockOuter = %u", fusedOuter, blockFactor,
              blockOuter);
    fBaseParams.blockOuter = blockOuter;
    fBaseParams.blockFactor = blockFactor;
    ...
    for (int64_t c = 0; c < blockOuter; c++) {
        OPS_LOG_D(context_, "blockNum[%d], blockStarts = %d , blockEnds = %d ", c, blockStarts[c],
                  blockEnds[c]);
    }
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2gs1s2::DoPostTiling()
{
    ...
    int64_t postUbBaseSize = (fBaseParams.ubSize - nzReservedSize) / curPostCoexNode / BUFFER_NUM /
