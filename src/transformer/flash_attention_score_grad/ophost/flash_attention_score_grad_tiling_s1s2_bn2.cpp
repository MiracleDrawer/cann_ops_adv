...
void FlashAttentionScoreGradTilingS1s2Bn2::SetQKVStartIdx() {
    ...
    if (qStartIdxShape.GetDimNum() != 1) {
        OPS_LOG_W(context_, "[%s]qStartIdxShape is invalid %ld %ld", templateNameBn2, qStartIdxShape.GetDimNum(),
    ...
    if (kvStartIdxShape.GetDimNum() != 1) {
        OPS_LOG_W(context_, "[%s]kvStartIdxShape is invalid %ld %ld", templateNameBn2, kvStartIdxShape.GetDimNum(),
...
bool FlashAttentionScoreGradTilingS1s2Bn2::SparseTokenProcess()
{
    ...
    if (sparseMode == NO_MASK) {
        if (int32_t(td_.opInfo.get_S1()) <= preTokens && nextTokens == 0) { // 原causal 场景
            ...
            OPS_LOG_I(context_,
                      "FAG S1s2Bn2 sparseMode[%d] with s1[%u] s2[%u] preToken[%d] nextToken[%d] is not support!",
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2::DoBlockTiling()
{
    ...
    for (uint32_t i = 0; i < td_.opInfo.get_usedCoreNum(); i++) {
        OPS_LOG_D(context_->GetNodeName(),
                  "FlashAttentionScoreGradTilingS1s2Bn2 splite B*N2 CoreIdx: %u begin: %u end: %u.", i,
...
{
    OPS_LOG_I(context_, "FAG S1s2Bn2 with shape b[%u] n2[%u] g[%u] s1[%u] s2[%u] d[%u] preToken[%d] nextToken[%d]!",
